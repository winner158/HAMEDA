...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.is_lecs = problem.NAME == 'lecs'
        ...
        if self.is_hcvrp:
            ...
            num_veh = 3
            node_dim = 2 + num_veh  # x,y, demand(3 vehicles)
            node_veh = 9
        ...
        if self.is_lecs:
            # Embedding of last node + remaining_capacity  & remaining duration to collect
            step_context_dim = embedding_dim + 2
            num_veh = 3
            # (1)step 1:增加duration
            node_dim = 2 + 2*num_veh  # x,y, demand(3 vehicles),duration(3 vehicles)
    ...
    def forward(self, input, return_pi=False):
        ...
        _log_p, log_p_veh, pi, veh_list, tour_1, tour_2, tour_3 = self._inner(input, embeddings)  # _log_p: [batch_size, graph_size+1, graph_size+1], pi:[batch_size, graph_size+1]

        cost, mask = self.problem.get_costs(input, self.obj, pi, veh_list, tour_1, tour_2, tour_3)  # mask is None, cost:[batch_size]

        # Log likelyhood is calculated within the model since returning it per action does not work well with
        # DataParallel since sequences can be of different lengths
        ll, ll_veh = self._calc_log_likelihood(_log_p, log_p_veh, pi, mask, veh_list)  # [batch_size]
        if return_pi:
    ...
    def _init_embed(self, input):
        ...
        if self.is_lecs:
            ...
            demand = torch.tensor([(input['demand'] / input['capacity'][0:1, veh]).tolist() for veh in
                                   range(input['capacity'].size(-1))]).transpose(0, 1).transpose(1, 2).cuda()
             # (1)step 2:增加duration
            duration = torch.tensor([(input['duration'] / input['max_duration'][0:1, veh]).tolist() for veh in
                                   range(input['max_duration'].size(-1))]).transpose(0, 1).transpose(1, 2).cuda()
            ...
            return torch.cat(  # [batch_size, graph_size+1, embed_dim]
                (
                    ...
                    self.init_embed(torch.cat((  # [batch_size, graph_size, embed_dim]
                        input['loc'],  # [batch_size, graph_size, 2]
                        demand,  # [batch_size, graph_size, num_veh]
                        duration # [batch_size, graph_size, num_veh]
                    ), -1))
                ),
                1
            )
    ...
    def _inner(self, input, embeddings):
        ...
        while not (self.shrink_size is None and state.all_finished()):
            veh, log_p_veh = self.select_veh(input, state, sequences, embeddings, self.obj, veh_list, tour_1, tour_2, tour_3)  # [batch_size, 1]
    ...
    def _get_log_p(self, fixed, state, veh, normalize=True):
        ...
        # print("\n glimpse_K shape:", glimpse_K.shape)
        #  glimpse_K shape: torch.Size([8, 128, 1, '25', 16])
        # print("\n glimpse_V shape:", glimpse_V.shape)
        # glimpse_V shape: torch.Size([8, 128, 1, 25, 16])
            ...
            if from_depot:
                ...
                return torch.cat(  # [batch_size, num_veh, embed_dim+1]
                    (
                        ...
                        (torch.tensor(self.problem.VEHICLE_CAPACITY)[None, veh].to('cuda') - state.used_capacity[torch.arange(batch_size), veh]).transpose(0, 1).unsqueeze(-1)
                    ),
                    -1
                )
            ...
            if from_depot:
                ...
                return torch.cat(  # [batch_size, num_veh, embed_dim+1]
                    (
                        ...
                        torch.tensor(self.problem.VEHICLE_CAPACITY)[None, :, None].cuda() - torch.zeros_like(
                            state.used_capacity[:, :, None]),
                            # (1)step 3:增加duration
                        torch.tensor(self.problem.MAX_DURATION)[None, :, None].cuda() - torch.zeros_like(
                            state.used_duration[:, :, None]),
                    ),
                    -1
                )
            else:
                ...
                return torch.cat(  # [batch_size, num_veh, embed_dim+1]
                    (
                        ...
                        (torch.tensor(self.problem.VEHICLE_CAPACITY)[None, veh].to('cuda') - state.used_capacity[torch.arange(batch_size), veh]).transpose(0, 1).unsqueeze(-1),
                         # (1)step 4:增加duration
                        (torch.tensor(self.problem.MAX_DURATION)[None, veh].to('cuda') - state.used_duration[torch.arange(batch_size), veh]).transpose(0, 1).unsqueeze(-1)
                    ),
                    -1
                )
    ...
    def _one_to_many_logits(self, query, glimpse_K, glimpse_V, logit_K, mask, veh):
        ...
        # print("\n glimpse_Q shape:", glimpse_Q.shape)
        ...
        # print("\n compatibility shape:", compatibility.shape)
        ...
        if self.mask_inner:  # True
            ...
            compatibility[mask[None, :, :, None, :].expand_as(compatibility)] = -math.inf  # mask visited nodes and nodes cannot be visited
